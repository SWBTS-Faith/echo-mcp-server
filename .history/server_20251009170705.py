...
async def health_check(request):
    return JSONResponse({"status": "healthy", "service": "echo-prayer-mcp-server"})

# Create ASGI app from MCP server
mcp_app = mcp.http_app(transport="streamable-http")

# Create a main FastAPI app that includes both MCP routes and root health endpoint
app = FastAPI(title="Echo Prayer MCP Server", lifespan=mcp_app.lifespan)
